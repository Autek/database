[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/oy32BVhp)
[![Open in Visual Studio Code](https://classroom.github.com/assets/open-in-vscode-2e0aaae1b6195c2367325f4f02e2d04e9abb55f0b24a779b69b11b9e10269abc.svg)](https://classroom.github.com/online_ide?assignment_repo_id=18888626&assignment_repo_type=AssignmentRepo)
# Lab environment
...
- `WritePage(block_id id, const char* page_data, StorageManager *storage_manager)`: Write the data pointed by `page_data` to the `id` th block of the file managed by the `storage_manager`. You can safely assume `page_data` points to an array of bytes which is at least `PAGE_SIZE`. You can also assume the file managed by the `storage_manager` has already been opened. When there is no error, return the number of bytes 
written (which should be `PAGE_SIZE`). There are several cases where you need to return an error, which should be processed in order:
  - `-ESTORAGEMANAGER` : `storage_manager` is `NULL`
  - `-ESTORAGEWRITE`: `id` is negative or `page_data` is `NULL`. If seeking a block or writing to a block returns any error, also return `-ESTORAGEWRITE`
- `ReadPage(block_id id, const char* page_data, StorageManager *storage_manager)`: Read the data from the `id` th block of file managed by the `storage_manager` and store the data in `page_data`. You can safely assume `page_data` points to an array of bytes which is at least `PAGE_SIZE`. You can also assume the file managed by `storage_manager` has already been opened. When there is no error, return the number of bytes read. If the file ends before reading `PAGE_SIZE` bytes, fill in the rest of the `page_data` array with 0. There are several cases where you need to return error, which should be processed in order:
  - `-ESTORAGEMANAGER` : `storage_manager` is `NULL`
  - `-ESTORAGEREAD` : `id` is negative or `page_data` is `NULL`. If the offset of the `id` th block from the beginning of the `file` is larger than the file size, return `-ESTORAGEREAD`. If seeking the block or reading from this block returns any error, also return `-ESTORAGEREAD`
...
- `bool FlushPage(block_id id, BufferManager *bm)`: Write the `Page` containing this `id`th block back to disk. Return false if the `Page` cannot be found in the buffer manager. After flushing the page, the page is not dirty.
...
```
## Submit
You must submit (commit) your code for lab2 before Monday, 5th of May 2025, 23:59.
...
If you encountered this when running gtest, this is most likely due to a bug in gtest. You can try out the following solution:
```bash
...
make
```
For further informaton, you may check this [link](https://github.com/quantumlib/Stim/issues/717) 